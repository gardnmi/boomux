...
impl Window {
    ...
    /// Paint through a parallelogram with horizontal top/bottom edges.
    /// `left` and `right` are absolute coordinates at the vertical midpoint;
    /// `slant` is the horizontal displacement from top to bottom. Text and
    /// layout stay upright. A nested call temporarily replaces the outer slant;
    /// rectangular masks continue to intersect normally. Paint phase only.
    pub fn with_slanted_content_mask<R>(
        &mut self,
        bounds: Bounds<Pixels>,
        left: Pixels,
        right: Pixels,
        ...
        let mask = crate::scene::SlantedMask {
            bounds: bounds.scale(scale),
            left: f32::from(left) * scale,
            right: f32::from(right) * scale,
            slant: f32::from(slant) * scale,
        };
        let previous = self.next_frame.scene.slanted_mask.replace(mask);
        let result = f(self);
        self.next_frame.scene.slanted_mask = previous;
        result
    }

    /// Paint within the intersection of the current and supplied rectangular masks.
