...
/// A paint-only parallelogram mask in physical pixel coordinates.
#[derive(Clone, Copy)]
pub(crate) struct SlantedMask {
...
pub struct Scene {
    ...
    pub(crate) slanted_mask: Option<SlantedMask>,
...
impl Scene {
    ...
    pub fn insert_primitive(&mut self, primitive: impl Into<Primitive>) {
        ...
        if let Some(mask) = self.slanted_mask {
            self.insert_slanted(primitive, mask);
        } else {
            self.insert_unmasked(primitive);
        }
    }

    fn insert_slanted(&mut self, primitive: Primitive, mask: SlantedMask) {
        // Markers must remain paired even where the visible region is empty.
        if matches!(primitive, Primitive::FilterBoundary(_)) {
            self.insert_unmasked(primitive);
            return;
        }
        ...
        if bounds.is_empty() || mask.right <= mask.left {
            return;
        }
        for [left, top, right, bottom] in slanted_mask::clip_bands(
            [
                bounds.left().0,
                bounds.top().0,
                bounds.right().0,
            ...
            part.content_mask_mut().bounds = Bounds::new(
                point(ScaledPixels(left), ScaledPixels(top)),
                Size::new(ScaledPixels(right - left), ScaledPixels(bottom - top)),
            );
            self.insert_unmasked(part);
        }
    }

    fn insert_unmasked(&mut self, mut primitive: Primitive) {
...
impl Primitive {
    ...
    fn content_mask_mut(&mut self) -> &mut ContentMask<ScaledPixels> {
        match self {
            ...
            Primitive::Path(p) => &mut p.content_mask,
            ...
            Primitive::BackdropFilter(p) => &mut p.content_mask,
            Primitive::FilterBoundary(p) => &mut p.content_mask,
        }
    }
