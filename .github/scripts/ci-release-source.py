...
def metadata(directory, sha, tag, target, kind="cli"):
    ...
    return {"kind": kind, "sha": sha, "tag": tag, "target": target, "sha256": digest}


def process(mode, directory, sha, tag, target, kind="cli"):
    expected = metadata(directory, sha, tag, target, kind)
...
if __name__ == "__main__":
    mode, directory, sha, tag, target, *kind = sys.argv[1:]
    process(mode, Path(directory), sha, tag, target, *kind)
