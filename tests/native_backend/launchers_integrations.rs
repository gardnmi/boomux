...
fn guided_setup_installs_omarchy_plugin_and_managed_bindings() {
    ...
    let config = fs::read_to_string(root.join("config/boomux/config.toml")).unwrap();
    assert!(config.contains("workspace_layer = \"hyprland-special\""));
