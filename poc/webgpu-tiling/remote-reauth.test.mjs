...
try{
 ...
 const info={node_id:'local',workspace_id:'home',nodes:[{id:'remote-owner',alias:'omarchy',health:'authentication_required',current:false,stale:true}],snapshot:{workspaces:[home]}};
 let setup,serial=0;
 ...
 await page.route('**/api/resource',r=>{
  ...
  if(op.action==='finish_setup'){
   ...
   return r.fulfill({json:{pending:false,shell:null}});
  }
 });
 await page.goto(process.env.POC_URL||'http://127.0.0.1:4387');await page.waitForSelector('.workspace-button');
 async function start(){await page.getByRole('tab',{name:'Remotes',exact:true}).click();if(serial===1)await page.getByRole('button',{name:'omarchy Sign-in required',exact:true}).click();await page.getByRole('button',{name:'Sign in…',exact:true}).click();await page.waitForFunction(()=>document.querySelector('.workspace-group.current')?.textContent.includes('Setup'));await page.getByRole('button',{name:'Take control',exact:true}).first().waitFor();}
 async function refresh(){await page.locator('#header-more summary').click();await page.locator('#header-refresh').click();}
 await start();setup.shells=[];await refresh();await page.waitForFunction(()=>document.querySelector('.workspace-group.current')?.textContent.includes('Home'));
 ...
 assert.equal(ops.filter(op=>op.action==='finish_setup').length,2);assert.deepEqual(ops.filter(op=>op.action==='guided'),[{action:'guided',workflow:'reauthenticate',owner:'remote-owner'},{action:'guided',workflow:'reauthenticate',owner:'remote-owner'}]);assert.deepEqual(errors,[]);
 console.log('Remote sign-in from collapsed/expanded cards targets the exact owner; completion restores the original Workspace');
