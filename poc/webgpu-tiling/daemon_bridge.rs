// PoC primary-controller bridge, derived from src/web_terminal.rs.
// Keep the production mobile collaborator bridge unchanged.
use std::io;
...
pub(crate) async fn run(socket: WebSocket, client: Client, grant: Grant) {
    ...
    loop {
        ...
        // Keep replay frames below the bounded WebSocket write buffer.
        for chunk in attachment.reconstruction.chunks(64 * 1024) {
            if !send_message(&mut sender, Message::Binary(chunk.to_vec().into())).await {
                return;
            }
        }
        drop(attachment.reconstruction);
...
async fn attach_exact(
    ...
    for _ in 0..RECONNECT_ATTEMPTS {
        ...
        match tokio::task::spawn_blocking(move || {
            if client.node_identity()? != grant.node_id {
            ...
            message = receiver.next() => match message {
                ...
                Some(Ok(Message::Text(text))) => match serde_json::from_str::<ClientMessage>(&text) {
                    Ok(ClientMessage::Resize { rows, cols, pixel_width, pixel_height })
                        if (1..=200).contains(&rows) && (2..=500).contains(&cols) =>
                    {
...
fn attachment_error(error: &client::ClientError) -> (&'static str, &'static str) {
    if matches!(
        ...
        (
            "busy",
            "Another terminal controls this Shell. Take control explicitly to resize it here.",
        )
    } else if exact_run_ended(error) {
        ("run_changed", "The ShellRun is no longer current")
    } else {
        ("attachment_failed", "Boomux could not attach to this Shell")
    }
}
